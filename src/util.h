...
namespace util
{
    ...
    template <class T>
    void vec2D_to_stl( vector2D<T> &v2D, vector<vector<T> > &v ){
        assert( v2D.xDim()==v.size() && v2D.yDim()==v[0].size() );
        #pragma omp parallel for
        for( int i=0; i<v2D.xDim(); ++i ){
            for( int j=0; j<v2D.yDim(); ++j ){
                v[i][j] = v2D(i,j);
            }
        }
    }
    template <class T>
    void stl_to_vec2D( vector<vector<T> > &v, vector2D<T> &v2D ){
        assert( v2D.xDim()==v.size() && v2D.yDim()==v[0].size() );
        #pragma omp parallel for
        for( int i=0; i<v2D.xDim(); ++i ){
            for( int j=0; j<v2D.yDim(); ++j ){
